...
Builder.load_file('view/screens/settings/ArchiveTestFoldersScreen.kv')

FOLDERNAME = "TestArchive/"+str(config.get('selected_folder',0))

class ArchiveFolder(SingleSelectableListBehavior, Label):
    pass


class ArchiveNavButton(Button):
    pass


class ArchiveFolderList(SingleSelectableList):
    def update(self, k, val):
        self.data = [{'text': str(x)} for x in self.list_data]


class ArchiveTestFoldersScreen(BaseScreen):
    ...
    def on_pre_enter(self):
        self.test_folder_names = [f for f in listdir("TestArchive") if (
            isdir(join("TestArchive", f)) and f != ".gitignore")]
    ...
    def go_back(self, obj):
        super(ArchiveTestFoldersScreen, self).back()
    ...
    def view_tests(self, obj):
        ...
        self.parent_screen.move_to('archive_screen')

    def rename_folder(self,obj):
        selected = self.folder_list.get_selected()
        print("self folder list: ", self.folder_list.list_data)
        config.set('selected_folder', str(selected[0]))
